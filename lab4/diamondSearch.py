...
class DiamondSearch(search.Search):
    ...
    def blockSAD(self, ref_center):
        ...
        for (x, y) in [(x, y) for x in range(self.n) for y in range(self.n)]:
            ...
            _sum += abs(self.current_picture[curr[1]][curr[0]] - self.referenced_picture[int(ref[1])][int(ref[0])])
    ...
    def motionEstimation(self):
        num_of_macroblocs_in_y=int(len(self.current_picture)/self.n)
        num_of_macroblocs_in_x=int(len(self.current_picture[0])/self.n)
        result=[]
                ...
                for x_ in range(num_of_macroblocs_in_x):
                    ...
                    for n1 in range(self.n):
                        for m1 in range(self.n):
                            ...
                            cy = y + n1
                            cx = x + m1
                            compressedImage[int(ey)][int(ex)] = self.current_picture[int(cy)][int(cx)]
