...
class Search:
    ...
    def __init__(self, current_picture, referenced_picture, n=2, p=2,useIntrpolation=False):
    ...
    def motionEstimation(self):
        ...
        num_of_macroblocs_in_y = int(len(current_picture) / self.n)
        num_of_macroblocs_in_x = int(len(current_picture[0]) / self.n)
        result = []
    ...
    def createCompressedImage(self):
        ...
        try:
            for y_ in range(int(num_of_macroblocs_in_y)):
                for x_ in range(int(num_of_macroblocs_in_x)):
                    x = x_ * self.n
                    y = y_ * self.n
